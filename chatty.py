...
MAINIDURL = 'file:///home/himanshu/Documents/'#please enter the data url if any
...
def greet( id ):
    response = requests.get(MAINIDURL + id + '.json')
    response_json = response.json()
    return float(response_json [0]['name'])

#Pairs is a list of patterns and responses.
idfind = [
    [
        r"(.*)",
        ["so your id is %1 ?"+ "".join( str(greet('2'))),]
    ]
]
pairs = [
    [
...
def greet():
	print("Please enter your full FSK ID")
	greet = Chat(idfind, reflections)
	greet.converse()
	chatty()
#Run the chatbot
greet()