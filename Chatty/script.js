...
function info_find(){
    ...
    for(var i = 0; i < SD.studentDetails.length; i++) {
        ...
        if (info['name'] === Uinput){
            ...
            print("Hello " + info_find.Name + " " + info_find.Surname + ", I am Chatty how may I help you today!", CT);
            print("If you need help with the commands type help. Type quit to leave\n\n\n",CT);
            print2("hi",['Menu','Todays Special'],['lol()','dude()'])
            change("chatty()");
            return 0;
        }
    }
    print("Please enter your first name only",CT)
}
...
function keycode(SC){
    ...
    Time = H + ":" + M;
    print("The route number is " + Routenumber,CT)
    print("The time is " + Time,CT)
    run()
}
...
function route(Stopname){
    ...
    print("Sorry That's not a route check your spellings or mistakes",CT);
}